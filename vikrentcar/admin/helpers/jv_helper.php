...
JS
	...
	public function renderVisualEditor($name, $value, $attrs = array(), $opts = array(), $btns = array())
	{
		...
		if (isset($modes['visual']) && isset($modes['modal-visual'])) {
			// change label to modal-visual
			$modes['modal-visual'] .= ' <i class="' . VikRentCarIcons::i('window-restore') . '"></i>';
		}
	...
	public function loadContextMenuAssets()
	{
		...
		$app_pref = VikRentCar::getAppearancePref();
		...
		$this->addScript(VRC_ADMIN_URI . 'resources/contextmenu.js');

		$doc = JFactory::getDocument();

		$doc->addStyleSheet(VRC_ADMIN_URI . 'resources/contextmenu.css');
		$doc->addScriptDeclaration(
<<<JS
(function($) {
	'use strict';
	...
	$(function() {
		$.vrcContextMenu.defaults.darkMode = {$dark_mode};
		$.vrcContextMenu.defaults.class    = 'vrc-dropdown-cxmenu';
	});
})(jQuery);
JS
		);
