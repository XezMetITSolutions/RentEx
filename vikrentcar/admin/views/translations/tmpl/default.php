...
<form action="index.php?option=com_vikrentcar&amp;task=translations" method="post" onsubmit="return vrcCheckChanges();">
	...
	if (!empty($active_table_key)) {
		...
		<div class="btn-group pull-left">
			<input type="text" name="keysearch" value="<?php echo JHtml::_('esc_attr', $keysearch); ?>" placeholder="<?php echo JHtml::_('esc_attr', JText::_('VRCDASHSEARCHKEYS')); ?>..." />
		</div>
		<?php
	}
	?>
...
foreach ($langs as $ltag => $lang) {
	...
	$flag = '';
	if (VRCPlatformDetection::isJoomla() && is_file(JPATH_SITE . DIRECTORY_SEPARATOR . 'media' . DIRECTORY_SEPARATOR . 'mod_languages' . DIRECTORY_SEPARATOR . 'images' . DIRECTORY_SEPARATOR . $lcountry . '.gif')) {
...
if (!empty($active_table_key)) {
	...
	if (!empty($keysearch)) {
		echo '<input type="hidden" name="keysearch" value="' . JHtml::_('esc_attr', $keysearch) . '"/>'."\n";
	}
}
foreach ($langs as $ltag => $lang) {
	$is_def = ($ltag == $vrc_tn->default_lang);
			...
			foreach ($values as $field => $def_value) {
				...
				?>
						...
						if ($type == 'text') {
							...
							if (VRCPlatformDetection::isWordPress() && interface_exists('Throwable')) {
								/**
		...
		<div class="vrc-translation-langcontent" style="display: none;" id="vrc_langcontent_ini">
			<fieldset class="adminform">
				...
				foreach ($all_inis as $initype => $inidet) {
					$inipath = $inidet['path'];
					?>
					<div class="vrc-translations-ini-element">
						<div class="vrc-translations-element-title">
							<div class="vrc-translate-element-cell"><?php echo JText::_('VRCINIEXPL'.strtoupper($initype)); ?></div>
						</div>
						<div class="vrc-translations-element-contents">
					<?php
					foreach ($langs as $ltag => $lang) {
						...
						?>
							<div class="vrc-translations-element-row <?php echo $t_file_exists ? 'vrc-field-translated' : 'vrc-missing-translation'; ?>">
								<div class="vrc-translations-element-lbl"><?php echo ($ltag == 'en-GB' ? 'Native ' : '').$lang['name']; ?></div>
								<div class="vrc-translations-element-val">
