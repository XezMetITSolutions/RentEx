...
JText::script('VRCINVGENERATING');
JText::script('VRCCONFIRMGENINV');
					...
					<div class="vrc-orderslist-cardetails">
						<div class="vrc-orderslist-cardetails-inner">
							<div class="vrc-orderslist-cardetails-carname">
								<span><?php echo $car ? $car['name'] : '-----'; ?></span>
							</div>
						<?php
...
// Invoices
$pconfirmgen = VikRequest::getInt('confirmgen', 0, 'request');
...
<div class="vrc-gen-invoices-helper" style="display: none;">
	<form action="index.php?option=com_vikrentcar" method="post" id="vrc-geninv-form">
		...
		<div class="vrc-calendar-cfield-entry">
			...
			<span>
				<select name="invoice_date" id="invoice_date">
					<option value="<?php echo date($df); ?>"><?php echo date($df); ?></option>
					<option value="0"><?php echo JHtml::_('esc_html', JText::_('VRCINVDATERES')); ?></option>
				</select>
			</span>
		</div>
		<div class="vrc-calendar-cfield-entry">
			<label for="company_info"><?php echo JText::_('VRCINVCOMPANYINFO'); ?></label>
			<span><textarea name="company_info" id="company_info" style="width: 98%; min-width: 98%; max-width: 98%; height: 70px;"><?php echo JHtml::_('esc_textarea', $companyinfo); ?></textarea></span>
		</div>
		<div class="vrc-calendar-cfield-entry">
			<label for="invoice_send"><?php VikRentCarIcons::e('envelope'); ?> <?php echo JText::_('VRCINVSENDVIAEMAIL'); ?></label>
			<span><select id="invoice_send" name="invoice_send"><option value=""><?php echo JText::_('VRNO'); ?></option><option value="1"><?php echo JText::_('VRYES'); ?></option></select></span>
		</div>
...
<script type="text/javascript">
	jQuery(function() {
		...
		var gen_invoices_modal_body = VRCCore.displayModal({
			suffix: 'invoices',
			extra_class: 'vrc-modal-rounded vrc-modal-tall',
			title: Joomla.JText._('VRCINVGENERATING').replace('%d', '<?php echo count($cid); ?>'),
			footer_right: '<button type="button" class="btn btn-success" onclick="document.getElementById(\'vrc-geninv-form\').submit();"><i class="vrcicn-file-text2"></i><?php echo addslashes(JText::_('VRCGENINVOICE')); ?></button>',
		<?php
		if (count($cid) === 1) {
			?>
			footer_left: '<a href="index.php?option=com_vikrentcar&task=editbusy&cid[]=<?php echo $cid[0]; ?>&frominv=1&return=order" class="btn vrc-config-btn"><?php echo addslashes(JText::_('VRMODRES')); ?></a>',
			<?php
		}
		?>
			onDismiss: () => {
				jQuery('#vrc-geninv-form').appendTo('.vrc-gen-invoices-helper');
			},
		});

		// set modal content
		jQuery('#vrc-geninv-form').appendTo(gen_invoices_modal_body);

		<?php
		if ($pconfirmgen > 0) {
			?>
		// prompt for immediate generation
		if (confirm(Joomla.JText._('VRCCONFIRMGENINV'))) {
			document.getElementById('vrc-geninv-form').submit();
		}
			<?php
		}
		?>
