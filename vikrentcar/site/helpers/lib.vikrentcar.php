...
class VikRentCar
{
	...
	public static function parseInvoiceTemplate($tmpl, $record, $car, $orderinfo)
	{
		...
		if ($usedcoupon !== false) {
			...
			$inv_rows .= '</tr>'."\n";
			$inv_rows .= '<tr>'."\n";
			$inv_rows .= '<td>' . JText::_('VRCCOUPON') . ': ' . $usedcoupon[2] . '</td>' . "\n";
			$inv_rows .= '<td></td>'."\n";
			$inv_rows .= '<td></td>'."\n";
...
HTML
			...
			foreach ($matches[1] as $reqf) {
				...
				if ($docheck) {
					foreach ($distinctive_features as $dfk => $dfv) {
						if (stripos($dfk, $reqf) !== false || (strtoupper($reqf) == $reqf && stripos($dfk, JText::_($reqf)) !== false) || preg_replace('/[^a-z0-9]/i', '', $dfk) == preg_replace('/[^a-z0-9]/i', '', $reqf)) {
							$feature_found = $dfk;
			...
			foreach ($pconfig as $value => $cont) {
				...
				switch ($cont['type']) {
					...
					case 'password':
						...
						$html .= '<input type="password" name="vikpaymentparams['.$value.']" value="'.(isset($arrparams[$value]) ? JHtml::_('esc_attr', $arrparams[$value]) : JHtml::_('esc_attr', $default_paramv)).'" size="20"/>';
					...
					case 'number':
						...
						$html .= '<input type="number" name="vikpaymentparams['.$value.']" value="'.(isset($arrparams[$value]) ? JHtml::_('esc_attr', $arrparams[$value]) : JHtml::_('esc_attr', $default_paramv)).'" ' . implode(' ', $number_attr) . '/>';
						break;
					case 'textarea':
						$html .= '<textarea name="vikpaymentparams['.$value.']">'.(isset($arrparams[$value]) ? JHtml::_('esc_textarea', $arrparams[$value]) : JHtml::_('esc_textarea', $default_paramv)).'</textarea>';
						break;
					case 'hidden':
						$html .= '<input type="hidden" name="vikpaymentparams['.$value.']" value="'.(isset($arrparams[$value]) ? JHtml::_('esc_attr', $arrparams[$value]) : JHtml::_('esc_attr', $default_paramv)).'"/>';
						break;
					case 'checkbox':
						...
						$html .= self::getVrcApplication()->printYesNoButtons('vikpaymentparams['.$value.']', JText::_('VRYES'), JText::_('VRNO'), (isset($arrparams[$value]) ? (int)$arrparams[$value] : (int)$default_paramv), 1, 0);
						break;
					default:
						$html .= '<input type="text" name="vikpaymentparams['.$value.']" value="'.(isset($arrparams[$value]) ? JHtml::_('esc_attr', $arrparams[$value]) : JHtml::_('esc_attr', $default_paramv)).'" size="20"/>';
						break;
				}
	...
	public static function displayCronParameters($file, $params = [])
	{
		...
		foreach ($config as $value => $cont) {
			...
			switch ($cont['type']) {
				...
				case 'password':
					$html .= '<div class="btn-wrapper input-append">';
					$html .= '<input type="password" name="vikcronparams['.$value.']" value="'.(isset($params[$value]) ? JHtml::_('esc_attr', $params[$value]) : JHtml::_('esc_attr', $default_paramv)).'" size="20"' . $inp_attr . '/>';
				...
				case 'number':
					...
					$html .= '<input type="number" name="vikcronparams['.$value.']" value="'.(isset($params[$value]) ? JHtml::_('esc_attr', $params[$value]) : JHtml::_('esc_attr', $default_paramv)).'" ' . implode(' ', $number_attr) . $inp_attr . '/>';
					break;
				case 'textarea':
					$html .= '<textarea name="vikcronparams['.$value.']"' . $inp_attr . '>'.(isset($params[$value]) ? JHtml::_('esc_textarea', $params[$value]) : JHtml::_('esc_textarea', $default_paramv)).'</textarea>';
					break;
				case 'visual_html':
					...
					break;
				case 'hidden':
					$html .= '<input type="hidden" name="vikcronparams['.$value.']" value="'.(isset($params[$value]) ? JHtml::_('esc_attr', $params[$value]) : JHtml::_('esc_attr', $default_paramv)).'"' . $inp_attr . '/>';
					break;
				case 'checkbox':
					// always display a hidden input value turned off before the actual checkbox to support the "off" (0) status
					$html .= '<input type="hidden" name="vikcronparams['.$value.']" value="0" />';
					$html .= self::getVrcApplication()->printYesNoButtons('vikcronparams['.$value.']', JText::_('VRYES'), JText::_('VRNO'), (isset($params[$value]) ? (int)$params[$value] : (int)$default_paramv), 1, 0);
					break;
				default:
					$html .= '<input type="text" name="vikcronparams['.$value.']" value="'.(isset($params[$value]) ? JHtml::_('esc_attr', $params[$value]) : JHtml::_('esc_attr', $default_paramv)).'" size="20"' . $inp_attr . '/>';
					break;
			}
			if (strlen($labelhelp) > 0) {
				$html .= '<span class="vrc-param-setting-comment">'.$labelhelp.'</span>';
	...
	public static function getLogBookingModification($old_booking) {
		...
		$prev_length = $old_booking['days'] . ' ' . ($old_booking['days'] > 1 ? JText::_('VRDAYS') : JText::_('VRDAY'));
