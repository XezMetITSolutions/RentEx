...
class VrcCurrencyConverter
{
	...
	protected function currencySymbol()
	{
		 ...
		 * @since 	1.15.5 (J) - 1.4.0 (WP)
		 */
		$custom_conv = VRCFactory::getPlatform()->getDispatcher()->filter('onGetCurrencyDataVikRentCar', [$this->to_currency]);
	...
	/**
	 * Main method called by the controller after invoking the class. The same method can also
	 * be called by other classes of VRC, and keeps associative arrays of rates to exchange.
	 * 
	 ...
	 * 
	 * @since 	1.15.5 (J) - 1.4.0 (WP) added argument to return plain exchanged floats.
	 */
	 ...
	 * 
	 * @since 	1.15.5 (J) - 1.4.0 (WP) the visibility is public, but used to be private before.
	 */
	public function getConversionRate()
	{
		 ...
		 * @since 	1.15.5 (J) - 1.4.0 (WP)
		 */
		$custom_conv = VRCFactory::getPlatform()->getDispatcher()->filter('onGetConversionRateVikRentCar', [$this->from_currency, $this->to_currency]);
	...
	public function currencyExists($currency_name)
	{
		 ...
		 * @since 	1.15.5 (J) - 1.4.0 (WP)
		 */
		$existing = VRCFactory::getPlatform()->getDispatcher()->filter('onCheckCurrencyExistsVikRentCar', [$currency_name]);
