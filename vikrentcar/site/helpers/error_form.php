...
if (VikRentCar::allowRent()) {
	...
	$coordsplaces = [];
	$selform = "<div class=\"vrcdivsearch vrcdivsearch-noresults\"><div class=\"vrcdivsearch-inner\"><form action=\"".JRoute::_('index.php?option=com_vikrentcar'.(!empty($pitemid) ? '&Itemid='.$pitemid : ''))."\" method=\"" . (VRCPlatformDetection::isWordPress() ? 'post' : 'get') . "\" onsubmit=\"return vrcValidateSearch();\">\n";
...
function vrcSetLocOpenTime(loc, where) {
	...
	jQuery.ajax({
		type: "POST",
		url: "' . VikRentCar::ajaxUrl(JRoute::_('index.php?option=com_vikrentcar&task=ajaxlocopentime&tmpl=component', false)) . '",
	...
	if ($diffopentime == true && $places && strlen((string)$places[0]['opentime'])) {
	...
	if (VRCPlatformDetection::isWordPress() || $calendartype == "jqueryui") {
		if ($vrcdateformat == "%d/%m/%Y") {
			$juidf = 'dd/mm/yy';
		} elseif ($vrcdateformat == "%m/%d/%Y") {
			$juidf = 'mm/dd/yy';
...
jQuery(function() {
	...
	jQuery('.vr-cal-img, .vrc-caltrigger').click(function() {
		...
		$selform .= "<div class=\"vrcsfentrycont\"><div class=\"vrcsfentrylabsel\"><label>" . JText::_('VRPICKUPCAR') . "</label><div class=\"vrcsfentrydate\">" . JHtml::_('calendar', '', 'pickupdate', 'pickupdate', $vrcdateformat, array (
			...
			'size' => '10',
			'maxlength' => '19'
		)) . "</div></div><div class=\"vrcsfentrytime\"><label>" . JText::_('VRALLE') . "</label><div class=\"vrc-sf-time-container\"><span id=\"vrccomselph\"><select name=\"pickuph\">" . $hours . "</select></span><span class=\"vrctimesep\">:</span><span id=\"vrccomselpm\"><select name=\"pickupm\">" . $minutes . "</select></span></div></div></div>\n";
		$selform .= "<div class=\"vrcsfentrycont\"><div class=\"vrcsfentrylabsel\"><label>" . JText::_('VRRETURNCAR') . "</label><div class=\"vrcsfentrydate\">" . JHtml::_('calendar', '', 'releasedate', 'releasedate', $vrcdateformat, array (
			...
			'size' => '10',
			'maxlength' => '19'
		)) . "</div></div><div class=\"vrcsfentrytime\"><label>" . JText::_('VRALLEDROP') . "</label><div class=\"vrc-sf-time-container\"><span id=\"vrccomseldh\"><select name=\"releaseh\">" . $hours . "</select></span><span class=\"vrctimesep\">:</span><span id=\"vrccomseldm\"><select name=\"releasem\">" . $minutes . "</select></span></div></div></div>";
	...
	if (empty($pcategory_id) && VikRentCar::showCategoriesFront()) {
		...
		if ($categories) {
			...
			//
			$selform .= "<div class=\"vrcsfentrycont\"><label for=\"vrc-categories\">" . JText::_('VRCARCAT') . "</label><div class=\"vrcsfentryselect\"><select name=\"categories\" id=\"vrc-categories\">";
	...
	//
	$selform .= "<div class=\"vrcsfentrycont\"><div class=\"vrcsfentrysubmit\"><input type=\"submit\" name=\"search\" value=\"" . JText::_('VRCSEARCHBUTTON') . "\" class=\"btn vrc-search-btn vrc-pref-color-btn\"/></div></div>\n";
	// close submit part
	$selform .= "</div>";
	//
	$selform .= "</div>\n";
	...
	if (count($coordsplaces) > 0) {
		$selform .= '<div class="vrclocationsbox"><div class="vrclocationsmapdiv"><a href="'.VikRentCar::externalRoute('index.php?option=com_vikrentcar&view=locationsmap&tmpl=component').'" class="vrcmodal" target="_blank"><i class="' . VikRentCarIcons::i('map-marked-alt') . '"></i> <span>'.JText::_('VRCLOCATIONSMAP').'</span></a></div></div>';
		?>
		...
		<script type="text/javascript">
			jQuery(document).ready(function() {
				var jqxhr = jQuery.ajax({
					type: "POST",
					url: "<?php echo VikRentCar::ajaxUrl(JRoute::_('index.php?option=com_vikrentcar&view=searchsuggestions&getjson=1&tmpl=component', false)); ?>",
