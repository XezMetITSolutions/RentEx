...
$navig = isset($this->navig) ? $this->navig : '';
$vrc_tn = $this->vrc_tn;
...
<div class="vrcstepsbarcont">
	<ol class="vrc-stepbar" data-vrcsteps="4">
		...
		<li class="vrc-step vrc-step-current"><span><?php echo JText::_('VRSTEPCARSELECTION'); ?></span></li>
		<li class="vrc-step vrc-step-next"><span><?php echo JText::_('VRSTEPOPTIONS'); ?></span></li>
...
<div class="vrc-search-results-block vrc-search-results-block-<?php echo VRCFactory::getConfig()->get('searchresstyle', 'list'); ?>">
<?php
foreach ($res as $k => $r) {
					...
					<div class="vrc-car-lastblock">
						...
						<div class="vrc-car-bookingbtn">
							...
							<form action="<?php echo JRoute::_('index.php?option=com_vikrentcar&view=vikrentcar'.(!empty($pitemid) ? '&Itemid='.$pitemid : '')); ?>" method="post">
					  		...
					  		?>
								<input type="submit" name="goon" value="<?php echo JText::_('VRPROSEGUI'); ?>" class="btn booknow vrc-pref-color-btn"/>
							</form>
						</div>
					</div>
				</div>
...
<div class="vrc-goback-block">
	<a href="<?php echo JRoute::_('index.php?option=com_vikrentcar&view=vikrentcar&pickup='.$pickup.'&return='.$release.(!empty($pitemid) ? '&Itemid='.$pitemid : '')); ?>" class="btn vrc-pref-color-btn-secondary"><?php echo JText::_('VRCHANGEDATES'); ?></a>
</div>
<?php
