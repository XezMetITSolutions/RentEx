...
class VikrentcarViewOconfirm extends JViewVikRentCar
{
	public function display($tpl = null)
	{
		...
		$pcarid = VikRequest::getInt('carid', 0, 'request');
			...
			if ($validtime === true) {
				if (!empty($ppriceid)) {
					...
					if ($dbo->getNumRows() == 1) {
						...
						showSelectVrc(JText::_('VRTARNOTFOUND'));
					}
				} else {
					showSelectVrc(JText::_('VRNOTARSELECTED'));
				}
			} else {
				showSelectVrc(JText::_('VRCARNOTCONS') . " " . date($df . ' H:i', $ppickup) . " " . JText::_('VRCARNOTCONSTO') . " " . date($df . ' H:i', $prelease));
			}
		} else {
