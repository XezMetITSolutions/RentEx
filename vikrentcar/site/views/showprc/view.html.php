...
class VikrentcarViewShowprc extends JViewVikRentCar
{
	public function display($tpl = null)
	{
		...
		$pcaropt = VikRequest::getInt('caropt', 0, 'request');
		...
		if (!$units) {
			VikError::raiseWarning('', JText::_('VRCARNOTFND'));
				...
				if ($dbo->getNumRows() == 1) {
					...
					if (intval($car[0]['avail']) == 1) {
						if (VikRentCar::dayValidTs($pdays, $ppickup, $prelease)) {
							...
							showSelectVrc(JText::_('VRERRCALCTAR'));
						}
					} else {
						showSelectVrc(JText::_('VRCARNOTAV'));
					}
				} else {
					showSelectVrc(JText::_('VRCARNOTFND'));
				}
			} else {
				showSelectVrc(JText::_('VRNOTARFNDSELO'));
			}
		} else {
			showSelectVrc(JText::_('VRCARNOTRIT') . " " . date($df . ' H:i', $ppickup) . " " . JText::_('VRCARNOTCONSTO') . " " . date($df . ' H:i', $prelease));
		}
	}
}
