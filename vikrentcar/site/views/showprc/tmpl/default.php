...
<div class="vrcstepsbarcont">
	<ol class="vrc-stepbar" data-vrcsteps="4">
		...
		<li class="vrc-step vrc-step-current"><span><?php echo JText::_('VRSTEPOPTIONS'); ?></span></li>
...
<form action="<?php echo JRoute::_('index.php?option=com_vikrentcar&view=vikrentcar'.(!empty($pitemid) ? '&Itemid='.$pitemid : '')); ?>" method="post" class="vrc-showprc-form">
	...
	<div class="vrc-showprc-prices-wrap">
		<h4 class="vrc-showprc-title"><?php echo JText::_('VRPRICE'); ?></h4>
...
if (!empty($car['idopt']) && is_array($optionals)) {
	...
	<div class="vrc-showprc-options-wrap">
		<h4 class="vrc-showprc-title"><?php echo JText::_('VRACCOPZ'); ?></h4>
		...
		foreach ($optionals as $k => $o) {
			...
			/**
			 * Maximum days of rent filter.
			 * 
			 * @since 	1.15.5 (J) - 1.4.0 (WP)
			 */
			if ($o['maxdays'] > 0 && $tars[$tindex]['days'] > $o['maxdays']) {
				continue;
			}
			...
			<div class="vrc-showprc-option-row">
				...
				if (!empty($o['img'])) {
					...
					<div class="vrc-showprc-option-img">
						<img alt="<?php echo $o['name']; ?>" src="<?php echo VRC_ADMIN_URI . 'resources/' . $o['img']; ?>" />
					</div>
					<?php
				}
				?>
...
?>
	...
	<div class="car_buttons_box">
		<input type="submit" name="goon" value="<?php echo JText::_('VRBOOKNOW'); ?>" class="btn booknow vrc-pref-color-btn"/>
