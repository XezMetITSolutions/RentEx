...
class VikRentCarController extends JControllerVikRentCar
{
	...
	public function saveorder()
	{
		...
		if (!$validtoken) {
			showSelectVrc(JText::_('VRINVALIDTOKEN'));
			return;
		}
		...
		if (!VikRentCar::dayValidTs($pdays, $ppickup, $prelease)) {
			showSelectVrc(JText::_('VRINCONGRDATA'));
			return;
		}
		...
		if (!$tar) {
			showSelectVrc(JText::_('VRINCONGRDATAREC'));
			return;
		}
		...
		if ($strisdue != $ptotdue) {
			showSelectVrc(JText::_('VRINCONGRTOT'));
			return;
		}
		...
		if (!($checkts <= $ppickup && $checkts < $prelease && $ppickup < $prelease)) {
			showSelectVrc(JText::_('VRINVALIDDATES'));
			return;
		}

		if (!$validlocations) {
			showSelectVrc(JText::_('VRINVALIDLOCATIONS'));
			return;
		}
		...
		if (!VikRentCar::carNotLocked($pcar, $units, $ppickup, $prelease)) {
			showSelectVrc(JText::_('VRCARISLOCKED'));
			return;
		}

		if (!VikRentCar::carBookable($pcar, $units, $ppickup, $prelease)) {
			showSelectVrc(JText::_('VRCARBOOKEDBYOTHER'));
			return;
		}
		...
		if (VikRentCar::areTherePayments()) {
			...
			if (is_array($payment)) {
				if (intval($payment['setconfirmed']) == 1) {
					...
					$app->redirect(JRoute::_("index.php?option=com_vikrentcar&view=order&sid=" . $sid . "&ts=" . $nowts . (!empty($pnodep) ? "&nodep={$pnodep}" : '') . (!empty($pitemid) ? "&Itemid=" . $pitemid : "") . ($ptmpl == 'component' ? '&tmpl=component' : ''), false));
					$app->close();
				} else {
				...
				$app->redirect(JRoute::_("index.php?option=com_vikrentcar&priceid=" . $ppriceid . "&place=" . $pplace . "&returnplace=" . $preturnplace . "&carid=" . $pcar . "&days=" . $pdays . "&pickup=" . $ppickup . "&release=" . $prelease . "&task=oconfirm" . ($ptmpl == 'component' ? '&tmpl=component' : '') . (!empty($pitemid) ? "&Itemid=" . $pitemid : ""), false));
				$app->close();
			}
		} else {
			$realback = VikRentCar::getHoursCarAvail() * 3600;
	...
	public function cancelrequest()
	{
		if (!JSession::checkToken()) {
			...
			VRCHttpDocument::getInstance()->close(403, JText::_('JINVALID_TOKEN'));
		}
	...
	public function ical()
	{
		...
		if (!empty($pkey) && $pkey == VikRentCar::getIcalSecretKey()) {
			...
			foreach ($rows as $r) {
				...
				$icalstr .= "SUMMARY:" . sprintf('%s #%d @ %s', JText::_('VRRENTALORD'), $r['id'], (string)$r['pickup_location_name']) . "\n";
	...
	public function order_upload_docs()
	{
		...
		$file_prefix = str_replace(' ', '-', JText::_('VRC_UPLOAD_DOCUMENTS')) . '_';
	...
	public function storedocsupload()
	{
		...
		$goto = JRoute::_('index.php?option=com_vikrentcar&view=order&sid=' . $order['sid'] . '&ts=' . $order['ts'] . (!empty($pitemid) ? '&Itemid=' . $pitemid : ''), false);
		...
		$app->enqueueMessage(JText::_('VRC_SUBMIT_DOCSUPLOAD_TNKS'));
		$app->redirect($goto);
	}
}
