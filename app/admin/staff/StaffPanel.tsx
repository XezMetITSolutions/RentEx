...
export default function StaffPanel({ currentStaff, initialStaff, locations }: StaffPanelProps) {
    ...
    const isFilialleiter = currentStaff.role === "FILIALLEITER";
    ...
    // Filter staff list based on role requirements:
    // FILIALLEITER cannot see ADMINISTRATORS or other FILIALLEITERS (can only see themselves, plus any MITARBEITER / FAHRER).
    const visibleStaff = staff.filter(s => {
        if (isFilialleiter) {
            // Hide all ADMINISTRATORs
            if (s.role === "ADMINISTRATOR") return false;
            // Hide other FILIALLEITERs
            if (s.role === "FILIALLEITER" && s.id !== currentStaff.id) return false;
        }
        return true;
    });
    ...
    function openNew() {
        ...
        setFormData({
            ...
            email: "",
            role: "MITARBEITER",
            locationId: isFilialleiter ? (currentStaff.locationId?.toString() ?? "") : "",
            password: "",
            isActive: true
        });
        setError("");
    ...
    async function save() {
        ...
        try {
            ...
            const payload = {
                ...formData,
                // If current staff is FILIALLEITER, lock the locationId to their own
                locationId: isFilialleiter ? currentStaff.locationId?.toString() : formData.locationId
            };
    ...
    async function toggleActive(s: StaffMember) {
        // Can only edit if not restricted
        if (isFilialleiter && (s.role === "ADMINISTRATOR" || s.role === "FILIALLEITER")) {
            return;
        }
    ...
    // Allowed roles to create/assign in form:
    const allowedRoles = isFilialleiter
        ? (["MITARBEITER", "FAHRER"] as const)
        : ROLES;
        ...
        <div className="p-6 md:p-8 min-h-screen bg-gray-50 dark:bg-black/50">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                {ROLES.map(role => {
                    ...
                    // For FILIALLEITER, hide count for admin or other managers to keep confidentiality
                    if (isFilialleiter && (role === "ADMINISTRATOR" || role === "FILIALLEITER")) {
                        return null;
                    }
                    return (
            ...
            <div className="bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-2xl overflow-hidden">
                <table className="w-full">
                    ...
                    <tbody>
                        {visibleStaff.map((s) => {
                            const isEditable = !isFilialleiter || (s.role !== "ADMINISTRATOR" && s.role !== "FILIALLEITER");
                            return (
                                <tr key={s.id} className="border-b border-gray-100 dark:border-white/5 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
                                    ...
                                    <td className="p-4">
                                        <button
                                            ...
                                            disabled={!isEditable}
                                            title={isEditable ? "Status umschalten" : "Keine Berechtigung"}
                            ...
                            <div className="p-6 space-y-4">
                                ...
                                <div>
                                    ...
                                    <select value={formData.role} onChange={e => setFormData(p => ({ ...p, role: e.target.value as Role }))}
                                        className="w-full px-3 py-2 border border-gray-300 dark:border-white/10 rounded-xl bg-white dark:bg-white/5 text-gray-900 dark:text-white text-sm focus:ring-2 focus:ring-red-500 outline-none">
                                        {allowedRoles.map(r => <option key={r} value={r} className="bg-white dark:bg-zinc-900 text-gray-900 dark:text-white">{r}</option>)}
                                    </select>
                                </div>
                                <div>
                                    ...
                                    <select
                                        ...
                                        disabled={isFilialleiter}
                                        className="w-full px-3 py-2 border border-gray-300 dark:border-white/10 rounded-xl bg-white dark:bg-white/5 text-gray-900 dark:text-white text-sm focus:ring-2 focus:ring-red-500 outline-none disabled:opacity-60 disabled:cursor-not-allowed"
                                    >
