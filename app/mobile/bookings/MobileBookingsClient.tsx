...
export default function MobileBookingsClient({ rentals }: { rentals: any[] }) {
  ...
  return (
    <div className="flex flex-col min-h-screen text-white pb-24 bg-[#0A0A0A]">
      ...
      {/* Header */}
      <header className="px-5 pt-12 pb-4 bg-[#141414] sticky top-0 z-20 shadow-sm border-b border-white/5">
        ...
        <div className="flex mt-6">
          <button 
            ...
            className={`flex-1 pb-3 text-[14px] font-medium text-center border-b-2 transition-colors ${
              activeTab === "Bevorstehend" ? "border-[#E53935] text-white" : "border-transparent text-[#A3A3A3]"
            }`}
          >
            Bevorstehend
          </button>
          <button 
            ...
            className={`flex-1 pb-3 text-[14px] font-medium text-center border-b-2 transition-colors ${
              activeTab === "Vergangen" ? "border-[#E53935] text-white" : "border-transparent text-[#A3A3A3]"
            }`}
          >
            Vergangen
          </button>
      ...
      <div className="px-5 mt-6 space-y-4">
        {displayedRentals.length > 0 ? (
          displayedRentals.map((rental) => (
            <div key={rental.id} className="bg-[#1C1C1C] rounded-[1.5rem] border border-white/5 overflow-hidden shadow-sm">
              <div className="relative h-32 w-full bg-black/20 flex items-center justify-center p-4">
                <Image 
