...
try {
    ...
    if (!in_array('license_plate', $columns)) {
        ...
        echo "Added license_plate column.<br>";
    }
    ...
    // Clear existing cars to avoid duplicates/mess for this demo (Optional, but ensures exact match)
    // $pdo->exec("TRUNCATE TABLE cars"); // Be careful with this in production! 
    // Instead, let's just insert if not exists or update based on license plate.
    ...
    foreach ($cars_data as $car) {
        // We need a unique key for ON DUPLICATE KEY UPDATE. Assuming license_plate is unique or we just insert.
        // If license_plate is not unique key, this might just insert duplicates. 
        ...
        if ($check->rowCount() > 0) {
            ...
            echo "Updated {$car['brand']} {$car['model']}<br>";
        } else {
            ...
            echo "Inserted {$car['brand']} {$car['model']}<br>";
        }
    }
