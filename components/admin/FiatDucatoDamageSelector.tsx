...
export default function FiatDucatoDamageSelector({ onChange }: Props) {
        ...
        <div className="space-y-6">
                ...
                <div className="flex border-b border-gray-100 dark:border-gray-800 overflow-x-auto hide-scrollbar">
                    {CAR_VIEWS.map((view) => (
                        <button
                            ...
                            className={`flex-1 px-4 py-3 text-xs font-bold transition-all whitespace-nowrap ${activeView === view.id
                    ...
                    {damages.map((d) => (
                        <div key={d.id} className="flex items-center gap-3 bg-white dark:bg-gray-900 p-3 rounded-2xl border border-gray-100 dark:border-gray-800">
                            ...
                            <div className="flex-1 min-w-0">
                                <p className="text-sm font-bold text-gray-900 dark:text-white truncate">{d.reason}</p>
            ...
            {modalOpen && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                    ...
                    <div className="relative bg-white dark:bg-gray-900 rounded-[2rem] shadow-2xl w-full max-w-lg overflow-hidden border border-white/20 animate-in fade-in zoom-in duration-200">
                        ...
                        <div className="p-6 space-y-4">
                            <div className="space-y-2">
                                <label className="text-[10px] font-black uppercase text-gray-400 tracking-widest px-1">Art</label>
                            ...
                            <div className="space-y-2">
                                <label className="text-[10px] font-black uppercase text-gray-400 tracking-widest px-1">Position</label>
                                <input
                                    type="text"
                                    value={location}
                                    onChange={(e) => setLocation(e.target.value)}
                                    placeholder="z.B. Kotflügel vorne links"
                                    className="w-full px-4 py-2 text-sm bg-gray-50 dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-800 outline-none focus:ring-2 focus:ring-blue-500"
                                />
                            </div>
                            ...
                            <div className="space-y-2">
                                ...
                                <div
                                    ...
                                    className={`relative h-24 rounded-2xl border-2 border-dashed flex flex-col items-center justify-center cursor-pointer transition-all overflow-hidden ${photoUrl ? 'border-green-500 bg-green-50 dark:bg-green-900/10' : 'border-gray-200 dark:border-gray-700 hover:border-blue-400'
                                        }`}
                                >
                                    {isUploading ? (
                                        ...
                                        <>
                                            ...
                                            <div className="absolute inset-0 bg-black/20 flex items-center justify-center"><Camera className="w-6 h-6 text-white" /></div>
                                        </>
                                    ) : (
                        ...
                        <div className="p-6 pt-0">
                            <button
                                ...
                                className="w-full py-3 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white font-bold rounded-xl shadow-lg shadow-blue-500/20 active:scale-[0.98] flex items-center justify-center gap-2"
                            >
            ...
            {viewingDamage && (
                <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
                    ...
                    <div className="relative bg-white dark:bg-gray-900 rounded-[2rem] shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-200">
                        <button type="button" onClick={() => setViewingDamage(null)} className="absolute top-4 right-4 p-2 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 rounded-full text-gray-500 z-10"><X className="w-4 h-4" /></button>
                        ...
                        <div className="p-6 space-y-4">
                            <div>
                                <span className="text-[10px] font-black uppercase tracking-widest text-blue-600">{CAR_VIEWS.find(v => v.id === viewingDamage.side)?.title}</span>
                                <h3 className="text-2xl font-black text-gray-900 dark:text-white uppercase">{viewingDamage.reason}</h3>
                            </div>
                            ...
                            <button type="button" onClick={() => { removeDamage(viewingDamage.id); setViewingDamage(null); }} className="w-full py-3 text-red-600 bg-red-50 hover:bg-red-100 font-bold rounded-xl transition-all">Löschen</button>
                        </div>
                    </div>
                </div>
            )}
