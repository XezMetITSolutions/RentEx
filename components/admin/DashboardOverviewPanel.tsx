...
import { Users, Car, Wallet, CalendarClock, Activity, Plus, ChevronRight } from 'lucide-react';
...
export default function DashboardOverviewPanel({
    ...
    // Map stats array to add links
    const enrichedStats = stats.map((stat) => {
        let linkUrl = '';
        if (stat.name === 'Gesamteinnahmen') {
            linkUrl = '/admin/rechnungen';
        } else if (stat.name === 'Aktive Vermietungen') {
            linkUrl = '/admin/reservations?status=Active';
        } else if (stat.name === 'Neue Kunden') {
            linkUrl = '/admin/customers';
        } else if (stat.name === 'Offene Reservierungen') {
            linkUrl = '/admin/reservations?status=Pending';
        }
        return { ...stat, linkUrl };
    });
        ...
        <div className="max-w-[1400px] mx-auto space-y-8 pb-10 px-4 sm:px-6">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-6 border-b border-gray-200 dark:border-gray-800">
                <div className="space-y-1">
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400">
                        Übersicht für <span className="font-medium text-gray-900 dark:text-gray-200">{staff?.location?.name || 'alle Standorte'}</span> am {currentDateStr}.
                    </p>
                </div>
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                {enrichedStats.map((stat) => {
                    const IconComponent = ICON_MAP[stat.icon] || Car;
                    return (
                        <div
                            key={stat.name}
                            onClick={() => stat.linkUrl && router.push(stat.linkUrl)}
                            className={clsx(
                                "bg-white dark:bg-gray-900 p-6 rounded-xl border border-gray-200 dark:border-gray-800 shadow-sm transition-all hover:border-gray-300 dark:hover:border-gray-700 cursor-pointer hover:shadow-md hover:-translate-y-0.5 transform duration-200"
                            )}
                        >
